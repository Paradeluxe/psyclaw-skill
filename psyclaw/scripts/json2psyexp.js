        ...
        for (const component of components) {
            if (component && component.enabled !== false) {
                if (component.type === 'audio') {
                    ...
                    parts.push(generateMouseComponentFromSchema(component, routineName));
                } else if (component.type === 'slider') {
                    parts.push(generateSliderComponentFromSchema(component, routineName));
                } else if (component.type === 'code') {
                    parts.push(generateCodeComponentFromSchema(component, routineName));
                }
            }
        }
    }
...
/**
 * 从 schema 格式生成 Mouse 组件
 */
function generateMouseComponentFromSchema(component, routineName) {
    const name = component.name || `${routineName}_mouse`;
    const startTime = convertTimeToSeconds(component.startTime) || '0.0';
    const duration = convertTimeToSeconds(component.duration);
    const saveMouseState = component.saveMouseState || 'final';
    const forceEndOnPress = component.forceEndRoutineOnPress || 'never';
    const timeRelativeTo = component.timeRelativeTo || 'mouse onset';
    const newClicksOnly = component.newClicksOnly !== false ? 'True' : 'False';
    const clickable = Array.isArray(component.clickable) ? component.clickable.join(', ') : (component.clickable || '');
    const saveParamsClickable = component.saveParamsClickable || 'name,';
    const storeCorrect = component.storeCorrect === true ? 'True' : 'False';
    const correctAns = Array.isArray(component.correctAns) ? component.correctAns.join(', ') : (component.correctAns || '');

    return `      <MouseComponent name="${name}" plugin="None">
        <Param val="${clickable}" valType="list" updates="constant" name="clickable"/>
        <Param val="${correctAns}" valType="list" updates="constant" name="correctAns"/>
        <Param val="False" valType="bool" updates="None" name="disabled"/>
        <Param val="" valType="code" updates="None" name="durationEstim"/>
        <Param val="${forceEndOnPress}" valType="str" updates="constant" name="forceEndRoutineOnPress"/>
        <Param val="${name}" valType="code" updates="None" name="name"/>
        <Param val="${newClicksOnly}" valType="bool" updates="constant" name="newClicksOnly"/>
        <Param val="True" valType="bool" updates="None" name="saveStartStop"/>
        <Param val="${saveMouseState}" valType="str" updates="constant" name="saveMouseState"/>
        <Param val="${saveParamsClickable}" valType="list" updates="set every repeat" name="saveParamsClickable"/>
        <Param val="" valType="code" updates="None" name="startEstim"/>
        <Param val="time (s)" valType="str" updates="None" name="startType"/>
        <Param val="${startTime}" valType="code" updates="None" name="startVal"/>
        <Param val="duration (s)" valType="str" updates="None" name="stopType"/>
        ...
        <Param val="${storeCorrect}" valType="bool" updates="constant" name="storeCorrect"/>
        <Param val="True" valType="bool" updates="constant" name="syncScreenRefresh"/>
        <Param val="${timeRelativeTo}" valType="str" updates="constant" name="timeRelativeTo"/>
      </MouseComponent>\n`;
}

/**
 * 从 schema 格式生成 Slider 组件
 */
function generateSliderComponentFromSchema(component, routineName) {
    const name = component.name || `${routineName}_slider`;
    const startTime = convertTimeToSeconds(component.startTime) || '0.0';
    const duration = convertTimeToSeconds(component.duration);
    const size = component.size || [1.0, 0.1];
    const pos = component.pos || [0, -0.3];
    const posX = pos[0] !== undefined && pos[0] !== null ? pos[0] : 0;
    const posY = pos[1] !== undefined && pos[1] !== null ? pos[1] : -0.3;
    const sizeX = size[0] !== undefined && size[0] !== null ? size[0] : 1.0;
    const sizeY = size[1] !== undefined && size[1] !== null ? size[1] : 0.1;
    const ori = component.ori !== undefined && component.ori !== null ? component.ori : 0;
    const units = component.units || 'from exp settings';
    const color = component.color || 'LightGray';
    const fillColor = component.fillColor || 'Red';
    const borderColor = component.borderColor || 'Black';
    const font = component.font || 'Arial';
    const letterHeight = component.letterHeight !== undefined && component.letterHeight !== null ? component.letterHeight : 0.03;
    const ticks = component.ticks !== undefined ? component.ticks : [1, 2, 3, 4, 5];
    const labels = Array.isArray(component.labels) ? component.labels : [];
    const initVal = component.initVal !== undefined && component.initVal !== null ? component.initVal : 0.5;
    const granularity = component.granularity !== undefined && component.granularity !== null ? component.granularity : 0;
    const forceEndRoutine = component.forceEndRoutine === true ? 'True' : 'False';
    const readOnly = component.readOnly === true ? 'True' : 'False';
    const storeRating = component.storeRating === true ? 'True' : 'False';
    const storeRatingTime = component.storeRatingTime === true ? 'True' : 'False';
    const storeHistory = component.storeHistory === true ? 'True' : 'False';
    const styles = component.styles || 'slider';
    const styleTweaks = Array.isArray(component.styleTweaks) ? component.styleTweaks : [];

    return `      <SliderComponent name="${name}" plugin="None">
        <Param val="${borderColor}" valType="color" updates="constant" name="borderColor"/>
        <Param val="${color}" valType="color" updates="constant" name="color"/>
        <Param val="False" valType="bool" updates="None" name="disabled"/>
        <Param val="" valType="code" updates="None" name="durationEstim"/>
        <Param val="${fillColor}" valType="color" updates="constant" name="fillColor"/>
        <Param val="${font}" valType="str" updates="constant" name="font"/>
        <Param val="${forceEndRoutine}" valType="bool" updates="constant" name="forceEndRoutine"/>
        <Param val="${granularity}" valType="num" updates="constant" name="granularity"/>
        <Param val="${initVal}" valType="num" updates="constant" name="initVal"/>
        <Param val="${JSON.stringify(labels).replace(/"/g, '&quot;')}" valType="list" updates="constant" name="labels"/>
        <Param val="${letterHeight}" valType="num" updates="constant" name="letterHeight"/>
        <Param val="${name}" valType="code" updates="None" name="name"/>
        <Param val="${ori}" valType="num" updates="constant" name="ori"/>
        <Param val="(${posX}, ${posY})" valType="list" updates="constant" name="pos"/>
        <Param val="${readOnly}" valType="bool" updates="constant" name="readOnly"/>
        <Param val="True" valType="bool" updates="None" name="saveStartStop"/>
        <Param val="${sizeX}, ${sizeY}" valType="list" updates="constant" name="size"/>
        <Param val="" valType="code" updates="None" name="startEstim"/>
        <Param val="time (s)" valType="str" updates="None" name="startType"/>
        <Param val="${startTime}" valType="code" updates="None" name="startVal"/>
        <Param val="duration (s)" valType="str" updates="None" name="stopType"/>
        <Param val="${duration}" valType="code" updates="constant" name="stopVal"/>
        <Param val="${storeHistory}" valType="bool" updates="constant" name="storeHistory"/>
        <Param val="${storeRating}" valType="bool" updates="constant" name="storeRating"/>
        <Param val="${storeRatingTime}" valType="bool" updates="constant" name="storeRatingTime"/>
        <Param val="${JSON.stringify(styleTweaks).replace(/"/g, '&quot;')}" valType="list" updates="constant" name="styleTweaks"/>
        <Param val="${styles}" valType="str" updates="constant" name="styles"/>
        <Param val="True" valType="bool" updates="constant" name="syncScreenRefresh"/>
        <Param val="${JSON.stringify(ticks).replace(/"/g, '&quot;')}" valType="list" updates="constant" name="ticks"/>
        <Param val="${units}" valType="str" updates="None" name="units"/>
      </SliderComponent>\n`;
}
...
/**
 * 从 schema 格式生成 Code 组件 (CodeComponent)
 *
 * Supported user YAML keys (mapped to PsychoPy params):
 *   - name           -> <CodeComponent name="...">  + Param name
 *   - code_type      -> Code Type  (Py/JS/Both/Auto->JS)
 *   - before_experiment / begin_experiment / end_experiment
 *   - before_routine  / begin_routine  / end_routine
 *   - each_frame
 *   - *_js_* mirrors (Before JS Experiment etc.)
 *
 * valType mapping: extendedCode for all code bodies (matches PsychoPy 2026.1.1).
 * Code text is HTML-escaped so user Python strings survive XML roundtrip.
 */
function generateCodeComponentFromSchema(component, routineName) {
    const name = component.name || `${routineName}_code`;
    const codeType = component.code_type || 'Py';
    ...
    const ph = (k) => esc(component[k] || '');

    return `      <CodeComponent name="${name}" plugin="None">
        <Param val="${ph('before_experiment')}" valType="extendedCode" updates="constant" name="Before Experiment"/>
        <Param val="${ph('before_js_experiment')}" valType="extendedCode" updates="constant" name="Before JS Experiment"/>
        <Param val="${ph('begin_experiment')}" valType="extendedCode" updates="constant" name="Begin Experiment"/>
        <Param val="${ph('begin_js_experiment')}" valType="extendedCode" updates="constant" name="Begin JS Experiment"/>
        <Param val="${ph('begin_js_routine')}" valType="extendedCode" updates="constant" name="Begin JS Routine"/>
        <Param val="${ph('begin_routine')}" valType="extendedCode" updates="constant" name="Begin Routine"/>
        <Param val="${codeType}" valType="str" updates="None" name="Code Type"/>
        <Param val="${ph('each_frame')}" valType="extendedCode" updates="constant" name="Each Frame"/>
        <Param val="${ph('each_js_frame')}" valType="extendedCode" updates="constant" name="Each JS Frame"/>
        <Param val="${ph('end_experiment')}" valType="extendedCode" updates="constant" name="End Experiment"/>
        <Param val="${ph('end_js_experiment')}" valType="extendedCode" updates="constant" name="End JS Experiment"/>
        <Param val="${ph('end_js_routine')}" valType="extendedCode" updates="constant" name="End JS Routine"/>
        <Param val="${ph('end_routine')}" valType="extendedCode" updates="constant" name="End Routine"/>
        <Param val="False" valType="bool" updates="None" name="disabled"/>
        <Param val="${name}" valType="code" updates="None" name="name"/>
      </CodeComponent>
`;
